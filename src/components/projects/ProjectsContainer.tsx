...
import { GET_PROJECTS } from "~/shared/graphql/project";
...
const ProjectsContainer: React.FC = () => {
  ...
  const { data, loading } = useQuery<QueryOutput<IProject[]>>(GET_PROJECTS);
  ...
  return (
    <PageWrapper>
      <div className="w-full grid grid-cols-4 auto-rows-max gap-4">
        <Dialog.Root>
          <Dialog.Trigger>
            <Button className="h-full py-8 px-10 cursor-pointer">+ Create a new project</Button>
          </Dialog.Trigger>
